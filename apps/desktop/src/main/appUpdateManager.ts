...
/**
 * 管理桌面更新的完整生命周期，并只向调用方暴露稳定快照和四个命令。
 *
 * 下载始终需要确认；已下载版本可以在用户主动重启或正常退出时安装。
 */
export class AppUpdateManager {
  ...
  start(): AppUpdateSnapshot {
    ...
    this.unsubscribe = this.options.updater.subscribe({
      ...
      error: (error) => {
        this.options.log(`桌面更新失败: ${safeErrorMessage(error)}`)
  ...
  async check(): Promise<AppUpdateSnapshot> {
    ...
    try {
      ...
      this.options.log(`桌面更新检查失败: ${safeErrorMessage(error)}`)
  ...
  async download(): Promise<AppUpdateSnapshot> {
    ...
    try {
      ...
      this.options.log(`桌面更新下载失败: ${safeErrorMessage(error)}`)
  ...
  async restartAndInstall(): Promise<AppUpdateSnapshot> {
    ...
    try {
      ...
      this.options.log(`桌面更新安装启动失败: ${safeErrorMessage(error)}`)
  ...
  private async offerDownload(snapshot: AppUpdateSnapshot): Promise<void> {
    ...
    try {
      ...
      this.options.log(`桌面更新下载确认失败: ${safeErrorMessage(error)}`)
    }
  }
  ...
  private async offerInstall(snapshot: AppUpdateSnapshot): Promise<void> {
    ...
    try {
      ...
      this.options.log(`桌面更新安装确认失败: ${safeErrorMessage(error)}`)
    }
  }
