...
describe('WorkbenchModeEntry', () => {
  it('matches the no-login industrial startup hierarchy', () => {
    ...
    expect(markup).toContain('aria-label="工作区目录"')
    expect(markup).toContain('输入工作区目录')
    expect(markup).not.toContain('选择文件夹')
    expect(markup).toContain('打开目录')
    expect(markup).toContain('/workspace/Uni-Lab-SZLab')
    expect(markup).toContain('进入调试模式')
    expect(markup).toContain('class="unilab-mode-entry__submit" type="button"')
    expect(markup).not.toContain('class="unilab-mode-entry__submit" type="submit"')
