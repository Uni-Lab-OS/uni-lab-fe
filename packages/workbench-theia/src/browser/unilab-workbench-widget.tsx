...
export class UniLabWorkbenchWidget extends ReactWidget {
  ...
  protected init(): void {
    ...
    this.addClass('unilab-workbench-widget')
...
function isWorkflowWorkbenchView(mode: WorkbenchViewMode): boolean {
  return mode === 'workflow-files' || mode === 'workflow-management-files' ||
    mode === 'workflow' || mode === 'workflow-management' ||
...
function workbenchViewLabel(mode: WorkbenchViewMode): string {
  if (mode === 'files') return '文件'
  if (mode === 'workflow-files') return '工作流调试 + 文件'
  if (mode === 'workflow-management-files') return '工作流管理 + 文件'
