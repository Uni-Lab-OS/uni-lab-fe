...
describe('WorkflowDag viewport authority', () => {
  ...
  it('fits once on initialization and preserves manual zoom across layout changes', () => {
    ...
    expect(canvasSource).not.toContain('resetCells(')
    expect(canvasSource).toContain('workflowX6ProjectionDiff(')
    expect(canvasSource).toContain("graph.batchUpdate('workflow-projection'")
