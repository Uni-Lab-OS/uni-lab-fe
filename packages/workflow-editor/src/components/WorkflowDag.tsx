...
function WorkflowDag({
  ...
  const nodeIdentityKey = JSON.stringify(nodes.map(node => node.id).sort())
  useEffect(() => {
    setAutomaticLayoutPreview(false)
    setPreviewPositions(null)
  }, [nodeIdentityKey])
  const [isBeautifying, setIsBeautifying] = useState(false)
  const [layoutStrategy, setLayoutStrategy] =
    useState<WorkflowDagLayoutStrategy>(DEFAULT_WORKFLOW_DAG_LAYOUT_STRATEGY)
  ...
  useImperativeHandle(forwardedRef, () => ({
    ...
    autoLayout: () => {
      setPreviewPositions(null)
      setLayoutStrategy('crossing-minimized')
      setSwimlaneDirection('horizontal')
      setAutomaticLayoutPreview(true)
    },
  ...
  const { nodes: layoutFlowNodes, edges: flowEdges } = useWorkflowDag(
    ...
    canvasMutationEnabled && !automaticLayoutPreview
  )
  const flowNodes = useMemo(() => previewPositions
    ? layoutFlowNodes.map(node => ({ ...node, position: previewPositions[node.id] ?? node.position }))
    : layoutFlowNodes, [layoutFlowNodes, previewPositions])
  const handleNodePositionChange = useCallback((nodeId: string, position: { x: number; y: number }) => {
    if (automaticLayoutPreview) {
      setPreviewPositions(current => ({
        ...(current ?? Object.fromEntries(flowNodes.map(node => [node.id, node.position]))),
        [nodeId]: position
      }))
    }
    onNodePositionChange?.(nodeId, position)
  }, [automaticLayoutPreview, flowNodes, onNodePositionChange])
  useEffect(() => {
    if (!automaticLayoutPreview || previewPositions) return
    const frame = requestAnimationFrame(() => x6CanvasRef.current?.fit())
    return () => cancelAnimationFrame(frame)
  }, [automaticLayoutPreview, flowNodes, previewPositions])
